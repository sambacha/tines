...
export class Edge {
    ...
    spentGasNew; //  How much gas was will be spent for this edge
    ...
    applySwap(from) {
        ...
        ASSERT(() => {
            if (this.direction) {
                ...
                return closeValues(this.amountOutPrevious / granularity, this.pool.calcOutByIn(this.amountInPrevious, this.direction).out / granularity, 1e-4);
            }
            else {
                const granularity = this.pool.granularity0();
                return closeValues(this.amountInPrevious / granularity, this.pool.calcOutByIn(this.amountOutPrevious, this.direction).out / granularity, 1e-4, `"${this.pool.address}" ${inPrev} ${to?.bestIncome} ${from.bestIncome}`);
            }
        }, `Error 225`);
    }
}
    ...
    topologySort(from, to) {
        ...
        function topSortRecursive(current) {
            ...
            for (let i = 0; i < outEdges.length; ++i) {
                const e = outEdges[i];
                const res = topSortRecursive(current.getNeighbor(e));
                if (res === 0)
                    return 0;
                if (res === 1) {
