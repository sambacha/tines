...
export class CLRPool extends RPool {
  ...
  calcOutByIn(amountIn: number, direction: boolean): { out: number; gasSpent: number } {
    ...
    while (input > 0) {
      ...
      if (direction) {
        const maxDx =
          (currentLiquidity * (currentPrice - nextTickPrice)) / currentPrice / nextTickPrice;
        //console.log('input, maxDx', input, maxDx);

        if (input <= maxDx) {
          output =
            (currentLiquidity * currentPrice * input) / (input + currentLiquidity / currentPrice);
          input = 0;
        } else {
        ...
        if (input <= maxDy) {
          ...
          output =
            (currentLiquidity * (nextTickPrice - currentPrice)) / currentPrice / nextTickPrice;
          currentPrice = nextTickPrice;
          input -= maxDy;
          if ((this.ticks[nextTickToCross].index / this.tickSpacing) % 2 === 0) {
            currentLiquidity += this.ticks[nextTickToCross].DLiquidity;
