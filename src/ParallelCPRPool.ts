...
export class ParallelCPRPool extends RPool {
  ...
  calcNextJumpforPool(
    ...
    const outputSecond =
      (combinedYNew * inputSecond) / (combinedYNew / poolPrice + inputSecond) + swapCost;
    ASSERT(() => {
      const outputSecond2 =
        ((combinedYNew + y) * inputSecond) / ((combinedYNew + y) / poolPrice + inputSecond);
  ...
  calcOutByIn(amountIn: number, direction: boolean): { out: number; gasSpent: number } {
    ...
    const addOutput =
      (jump.combinedLiquidityY * addInput) / (jump.combinedLiquidityY / jump.price + addInput);
  ...
  calcInByOut(amountOut: number, direction: boolean): { inp: number; gasSpent: number } {
    ...
    let addInput =
      ((jump.combinedLiquidityY / jump.price) * addOutput) / (jump.combinedLiquidityY - addOutput);
